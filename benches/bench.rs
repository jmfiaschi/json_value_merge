...
fn merge(c: &mut Criterion) {
    ...
    group.bench_function("merge_string", |b| {
        b.iter(|| black_box(json_value.clone().merge(Value::String("my string B".to_string()))))
    });
...
fn merge_in(c: &mut Criterion) {
    ...
    group.bench_function("merge_in_string.", |b| {
        b.iter(|| black_box(json_value.clone().merge_in("/field", Value::String("new value".to_string()))))
    });
    ...
    group.bench_function("merge_in_object", |b| {
        b.iter(|| {
            black_box({
                json_value.clone().merge_in("/field", json_value_to_merge.clone()).unwrap();
            })
        })
    });
    ...
    group.bench_function("merge_in_array", |b| {
        b.iter(|| {
            black_box({
                json_value.clone().merge_in("/0", json_value_to_merge.clone()).unwrap();
            })
        })
    });
    ...
    let json_value_to_merge: Value = serde_json::from_str(r#"{"new_field":"new_value"}"#).unwrap();
    group.bench_function("merge_in_deeper_object", |b| {
        b.iter(|| {
            black_box({
                json_value.clone().merge_in("/child/child/child/batters/batter", json_value_to_merge.clone()).unwrap();
            })
        })
    });
